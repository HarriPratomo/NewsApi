...
public class FragmentEntertainment extends Fragment {
    ...
    private List<NewsArticalItem> list_businness = new ArrayList<>();
    ...
    private void init(View view) {
        progressBar = view.findViewById(R.id.progress_entertainment);
        recyclerView = view.findViewById(R.id.rv_intertainment);
        apiInterface = RetrofitApi.getClient().create(ApiInterface.class);
        adapter = new Adapter_Toplines(list_businness, getContext());
        ...
        apiInterface.getListAllNewsCategory("id", category, Constants.API_KEY).enqueue(new Callback<NewsResponse>() {
            ...
            public void onResponse(Call<NewsResponse> call, Response<NewsResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    list_businness = response.body().getArticles();
                    adapter = new Adapter_Toplines(list_businness, getContext());
